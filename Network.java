...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        if (name == null || this.getUser(name) != null || userCount >= this.users.length) { // If ths network is full or  given a name that is already a user in this network
            return false;
        }
    ...
    public boolean addFollowee(String name1, String name2) {
        ...
        if (this.getUser(name1) == null || this.getUser(name2) == null) { // If one of the two names is not a user in this network
            return false;
        }
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i=1; i < userCount; i++) { // Going through all the users in the network
            if (name == this.users[i].getName()) {continue;} //  not searchig itself
            
            int thisUserCount = this.users[i].countMutual(this.getUser(name)); //how many Mutual friends, this user and the given user have
            if ( thisUserCount > mostRecommendedUserToFollowCount ) { // placing the new most recommended user
    ...
    public String mostPopularUser() {
        ...
        System.out.println("we have more than one");

        User mostPopular = this.users[0];
        int mostPopularUserCount = followeeCount(this.users[0].getName());
        for (int i=1; i < userCount; i++) { // Going through all the users in the network
            int thisUser = followeeCount(this.users[i].getName());
            if (thisUser > mostPopularUserCount) {
                mostPopular = this.users[i];
                mostPopularUserCount = thisUser;
            }
        }
